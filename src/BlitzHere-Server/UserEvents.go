...
func UpdateUserEvent(userID string, event *BlitzMessage.UserEvent) error {
    //Log.LogFunctionName()
    defer func() {
...
func UpdateUserEvents(writer http.ResponseWriter, userID string, userEvents *BlitzMessage.UserEventsRequest) {
    ...
    for i := 0; i < len(userEvents.UserEvents); i++ {
        ...
        if error == nil {
            ...
            Log.Errorf("Error updating event %s %v: %v.",
                userID, BlitzMessage.NullTimeFromTimestamp(userEvents.UserEvents[i].Timestamp), error)
        }
    }
    ...
    userEventResponse := &BlitzMessage.UserEventsResponse {
    ...
    response := &BlitzMessage.ServerResponse {
        ...
        Response:           &BlitzMessage.ServerResponse_UserEventResponse { UserEventResponse: userEventResponse},
    }
...
func SaveDebugMessages(writer http.ResponseWriter, userID string, debugMessage *BlitzMessage.DebugMessage) {
    Log.LogFunctionName()
