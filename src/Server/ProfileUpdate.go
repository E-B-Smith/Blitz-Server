...
func UpdateProfile(profile *happiness.Profile) error {
    ...
    userID, error := happiness.ValidateUserID(profile.UserID)
    if error != nil {
    ...
    _, error = config.DB.Exec("update usertable set" +
        ...
        profile.Name,
        profile.Gender,
        happiness.NullTimeFromTimestamp(profile.Birthday),
        profile.UserStatus,
        happiness.NullTimeFromTimestamp(profile.CreationDate),
        &userID)
    if error != nil {
...
func UpdateProfiles(writer http.ResponseWriter, userID string, profiles *happiness.ProfileUpdate) {
