...
type Session struct {
    ...
    Device              happiness.DeviceInfo
    AppOptions          *happiness.AppOptions
}
...
func UpdateSession(writer http.ResponseWriter, ipAddress string, sessionToken string, request *happiness.SessionRequest) {
    ...
    if request.Profile == nil {
        SendError(writer, happiness.ResponseCode_RCInputInvalid, nil)
        return
    }
    userID, error := happiness.ValidateUserID(request.Profile.UserID)
    if error != nil {
    ...
    if  request.DeviceInfo.AppVersion == nil ||
        ...
        SendError(writer, happiness.ResponseCode_RCClientTooOld, error)
        return
    }
    ...
    var invite *happiness.AcceptInviteRequest = nil
    if request.Profile.UserStatus == nil || *request.Profile.UserStatus < happiness.UserStatus_USActive {
        ...
        if invite != nil {
            ...
            if profile == nil {
                ...
                if senderProfile != nil {
                    invite.Profiles = [] *happiness.Profile { senderProfile }
                }
            }
        }
    }
    ...
    _, error = config.DB.Exec("insert into usertable (userid, userstatus) "+
      "  values ($1, $2);", userID, happiness.UserStatus_USActive)
    Log.Debugf("Result: %v.", error)

    var status happiness.UserStatus = happiness.UserStatus_USUnknown
    if profile.UserStatus == nil {
        profile.UserStatus = &status
    }

    Log.Debugf("Update user.")
    if *profile.UserStatus == happiness.UserStatus_USBlocked {
        Log.Warningf("Blocking user %s.", *profile.UserID)
        SendError(writer, happiness.ResponseCode_RCInputInvalid, error)
        return
    } else if *profile.UserStatus < happiness.UserStatus_USActive {
        _, error = config.DB.Exec(
            "update usertable set (lastseen, userstatus) = ($1, $2) where userID = $3;",
             time.Now(), happiness.UserStatus_USActive, userID)
        Log.Debugf("Result: %v.", error)
    } else {
        _, error  = config.DB.Exec(
    ...
    sessionResponse := &happiness.SessionResponse {
        UserID:             &userID,
    ...
    response := &happiness.ServerResponse {
        ResponseCode:   &code,
        Response:       &happiness.ServerResponse_SessionResponse { SessionResponse: sessionResponse },
    }
