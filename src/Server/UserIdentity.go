...
func IdentityStringsFromProfile(profile *happiness.Profile) []string {
    ...
    for _, contact := range profile.ContactInfo {
        if contact.ContactType != nil && contact.Contact != nil {
            ids = appendID(ids, happiness.ContactType_name[int32(*contact.ContactType)], *contact.Contact)
        }
    }
...
func UpdateUserIdentitesFromProfile(profile *happiness.Profile) {
...
func ExistingProfileFromIdentities(identities []string) *happiness.Profile {
    Log.LogFunctionName()
...
func ProfilesFromContactInfo(profilesIn []*happiness.Profile) []*happiness.Profile {
    Log.LogFunctionName()

    profileMap := make(map[string]*happiness.Profile)
    for _, profile := range profilesIn {
        ...
        if rowCount == 0 {
            ...
            status := happiness.UserStatus_USInvited
    ...
    profilesOut := make([]*happiness.Profile, 0, len(profileMap))
...
func ProfilesFromContactInfoRequest(writer http.ResponseWriter,
            session *Session,
            profilesFromContactInfo *happiness.ProfilesFromContactInfo) {
    Log.LogFunctionName()

    profileUpdate := happiness.ProfileUpdate { }
