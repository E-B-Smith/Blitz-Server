...
func UpdateSocialIdentityForUserID(userID string, social *happiness.SocialIdentity) error {
    Log.LogFunctionName()
    defer func() {
...
func SocialIdentitiesWithUserID(userID string)  []*happiness.SocialIdentity {
    Log.LogFunctionName()
    defer func() {
        if error := recover(); error != nil { Log.LogStackWithError(error) }
    } ()

    social := make([]*happiness.SocialIdentity, 0, 10)
