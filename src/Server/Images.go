...
func UploadImage(writer http.ResponseWriter, userID string, imageUpload *happiness.ImageUpload) {
    Log.LogFunctionName()

    if imageUpload.ImageData == nil || len(imageUpload.ImageData) == 0 {
        SendError(writer, happiness.ResponseCode_RCInputInvalid, errors.New("No image in message"))
        return
    }
    ...
    if len(imageData.ImageBytes) > 1000000 {
        SendError(writer, happiness.ResponseCode_RCInputInvalid, errors.New("Image > 1 megabyte"))
        return
    }
    ...
    if rowsUpdated != 1 {
        ...
        SendError(writer, happiness.ResponseCode_RCServerError, errors.New("UserTable error"))
        return
    }
    Log.Debugf("Updated user '%s' image to '%s'.", userID, imageURL)

    replyImageData := happiness.ImageData {
        ImageContent: imageData.ImageContent,
        ContentType:  imageData.ContentType,
        ImageURL:     &imageURL,
    }
    replyImageUpload := happiness.ImageUpload {
        ImageData:  []*happiness.ImageData{ &replyImageData },
    }
    code := happiness.ResponseCode_RCSuccess
    response := &happiness.ServerResponse {
        ResponseCode: &code,
        Response:     &happiness.ServerResponse_ImageUploadReply { ImageUploadReply:  &replyImageUpload },
    }
...
func GetImage(writer http.ResponseWriter, httpRequest *http.Request) {
    ...
    uid, error = happiness.ValidateUserID(&uid)
    if error != nil {
        http.Error(writer, "Not Found", 404)
        return
    }
