...
func UserIsConfirming(writer http.ResponseWriter, session *Session, confirmation *happiness.ConfirmationRequest) {
    Log.LogFunctionName()

    var verified bool = false
    var userStatus happiness.UserStatus = happiness.UserStatus_USConfirmed
    ...
    if ! verified {
        userStatus = happiness.UserStatus_USActive
    ...
    confirmed := happiness.ConfirmationRequest {
        Profile: profile,
    }
    ...
    response := &happiness.ServerResponse {
        ...
        Response:           &happiness.ServerResponse_ConfirmationRequest { ConfirmationRequest: &confirmed },
    }
...
func UserConfirmation(writer http.ResponseWriter, session *Session, confirmation *happiness.ConfirmationRequest) {
    Log.LogFunctionName()

    var error error
    profile := confirmation.Profile;
    if profile == nil {
        SendError(writer, happiness.ResponseCode_RCInputInvalid, errors.New("A profile is required."))
        return
    }
    if profile.Name != nil {
    ...
    if profile.Name == nil || len(*profile.Name) <= 0 {
        SendError(writer, happiness.ResponseCode_RCInputInvalid, errors.New("A name is required."))
        return
    }
    contact := confirmation.ContactInfo
    if contact == nil {
        SendError(writer, happiness.ResponseCode_RCInputInvalid, errors.New("Contact info is required."))
        return
    }
    if contact.Contact == nil {
        SendError(writer, happiness.ResponseCode_RCInputInvalid, errors.New("Contact info detail is required."))
        return
    }
    ...
    var userStatus happiness.UserStatus = happiness.UserStatus_USConfirming
    profile.UserStatus = &userStatus
    ...
    switch *contact.ContactType {
        //case happiness.ConfirmationType_CTFacebook:

        case happiness.ContactType_CTEmail:
            email, error := Util.ValidatedEmailAddress(*contact.Contact)
            if error != nil {
                SendError(writer, happiness.ResponseCode_RCInputInvalid, errors.New("A valid email address is required."))
                return
            }
        ...
        case happiness.ContactType_CTPhoneSMS:
            phone, error := Util.ValidatedPhoneNumber(*contact.Contact)
            if error != nil {
                SendError(writer, happiness.ResponseCode_RCInputInvalid, errors.New("A valid phone number is required."))
                return
            }
        ...
        default:
            SendError(writer, happiness.ResponseCode_RCInputInvalid, fmt.Errorf("Unknown confirmation type %d.", contact.ContactType))
            return
    }
...
func AcceptInviteRequest(writer http.ResponseWriter, session *Session, invite *happiness.AcceptInviteRequest) {
    //
    //  AcceptInvite
    //
    ...
    if  invite.ContactInfo == nil || invite.UserID == nil {
        SendError(writer, happiness.ResponseCode_RCInputInvalid, errors.New("No contact info"))
        return
    }

    currentProfile := ProfileForUserID(session.UserID)
    if currentProfile == nil {
        SendError(writer, happiness.ResponseCode_RCInputInvalid, errors.New("No Profile"))
        return
    }
    ...
    var status happiness.UserStatus = happiness.UserStatus_USConfirmed
    currentProfile.UserStatus = &status
    if currentProfile.ContactInfo == nil { currentProfile.ContactInfo = make([]*happiness.ContactInfo, 0, 1) }
    ...
    inviteResponse := happiness.AcceptInviteResponse {
    ...
    response := &happiness.ServerResponse {
        ResponseCode:   &code,
        Response:       &happiness.ServerResponse_AcceptInviteResponse { AcceptInviteResponse: &inviteResponse},
    }
