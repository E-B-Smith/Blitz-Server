...
func SystemMessageFormRequest(writer http.ResponseWriter, httpRequest *http.Request) {
    ...
    rows, error := config.DB.Query("select userID from UserTable where userStatus >= $1;", happiness.UserStatus_USActive);
    ...
    for rows.Next() {
        ...
        _, error := config.DB.Exec(
            ...
            messageID,
            happiness.Default_Globals_SystemUserID,
            recipientID,
            messageDate,
            happiness.MessageType_MTSystem,
            messageText,
            actionIcon,
            actionURL,
        )
